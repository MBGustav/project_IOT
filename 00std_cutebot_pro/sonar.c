...
void sonar_init(void) {
    NRF_P0->PIN_CNF[US_TRIG_PIN] = 0x00000003;   // output config
    NRF_P0->OUTCLR = (1U << US_TRIG_PIN);
    NRF_P0->PIN_CNF[US_ECHO_PIN] = 0x00000000;   // input config


    // Initialize TIMER0 for microsecond timing
...
static uint32_t fetch_current_time_us(void) {
...
int get_sonar_distance_cm(void) {
    ...
    uint32_t t = fetch_current_time_us();
    while (fetch_current_time_us() - t < 10);
    NRF_P0->OUTCLR = (1U << US_TRIG_PIN);

    // wait for echo HIGH (timeout 5 ms)
    t = fetch_current_time_us();
    while ((NRF_P0->IN & (1U << US_ECHO_PIN)) == 0) {
        if (fetch_current_time_us() - t > 5000) return 999;
    }

    uint32_t echo_start = fetch_current_time_us();

    // wait for echo LOW (timeout 25 ms ~ 430 cm)
    while (NRF_P0->IN & (1U << US_ECHO_PIN)) {
        if (fetch_current_time_us() - echo_start > 25000) return 999;
    }

    uint32_t echo_us = fetch_current_time_us() - echo_start;
